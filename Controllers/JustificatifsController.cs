...
namespace genetrix.Controllers
{
    public class JustificatifsController : Controller
    {
        ...
        {
            var getJustificatifs = db.GetJustificatifs.Include(j => j.DossiersReglParts);
        ...
        public async Task<ActionResult> Details(int? id)
        {
            ...
            Justificatif justificatif = await db.GetJustificatifs.Include(j=>j.DossiersReglParts).FirstOrDefaultAsync(j=>j.Id==id);
        ...
        public async Task<JsonResult> CreateJs(Justificatif justificatif)
        {
            ...
            Dossier dossier = null;bool newFacture = false;
            try
            {
                //if (ModelState.IsValid)
                //if (Request.Files.Count > 0)
                {
                    ...
                    if (justificatif.Id > 0)
                    {
                        justificatif = db.GetJustificatifs.Find(justificatif.Id);
                    }
                    else
                    {
                        db.GetJustificatifs.Add(justificatif);
                    ...
                    if(justificatif.DossiersReglParts==null)
                        justificatif.DossiersReglParts=new List<Dossier>();
                    if(justificatif.DossiersReglParts.Count>0)
                    {
                        dossier.NatureOperation = justificatif.DossiersReglParts.FirstOrDefault().NatureOperation;
                        dossier.Motif = justificatif.DossiersReglParts.FirstOrDefault().Motif;
                    }
                    justificatif.DossiersReglParts.Add(dossier);
                    await db.SaveChangesAsync();
                    ...
                    if (justificatif.EstAncienneFactMoins6mois == 1)
                    {
                        try
                        {
                            ...
                            await db.GetJustificatifs.Include(j=>j.DossiersReglParts).Include(j=>j.GetImages).Where(j => j.NumeroJustif == justificatif.NumeroJustif && j.Id!=justificatif.Id && j.IdClient == justificatif.IdClient).ForEachAsync(j =>
                            {
                    ...
                    if (Request.Files != null && newFacture)
        ...
        public ActionResult GetfactureCorrespondants(int DossierId, int fournisseurId, int deviseId,string num)
        {
            ...
            var dossCurrent = db.GetDossiers.Find(DossierId);
            if (jus.MontantRestant2 > 0)
            {
                string msg2 = "";
                if (dossCurrent.NatureOperation !=doss.NatureOperation || dossCurrent.Motif !=doss.Motif)
                {
                    msg2 = "La nature et le motif de l'instruction seront ajustés par rapport au précedent reglement après la validation de cette facture";
                }
                ...
                return Json(new { estPartiel = "1", reste = (jus.MontantRestant - dossCurrent.Montant).ToString(), devise = jus.Devise, montant = jus.MontantJustif, msg = msg, files =from g in jus.GetImages select g.Id, dossierId = doss.Dossier_Id, anneeFact=jus.DateEmissioJustif.Year, moisFact=jus.DateEmissioJustif.Month, jourFact=jus.DateEmissioJustif.Day, nbrePieces = jus.NbrePieces,IdJustif=jus.Id}, JsonRequestBehavior.AllowGet) ;
            }
            else
            {
                return Json(new { estPartiel = "0", reste = (jus.MontantRestant- dossCurrent.Montant).ToString(), devise = jus.Devise, montant = jus.MontantJustif, msg = msg, files = from g in jus.GetImages select g.Id, dossierId = doss.Dossier_Id, anneeFact = jus.DateEmissioJustif.Year, moisFact = jus.DateEmissioJustif.Month, jourFact = jus.DateEmissioJustif.Day, nbrePieces = jus.NbrePieces,IdJustif = jus.Id }, JsonRequestBehavior.AllowGet);
            }
        }
