...
{
    public class ArchivagesController : Controller
    {
        ...
        public ActionResult Index(int idDossier = 0,int idFournisseur = 0
            ...
            string tablist = "box",string cat="",bool inp_dfx=false,bool inp_fp = false,bool inp_ref = false,string tri="", bool dxk = false)
        {
            ViewBag.vue = vue;
            ...
            { 
                string str = _id;
                if (_id.Contains('-'))
                    for (int i = 1; i < _id.Split('-').Length; i++)
                    { 
                        if (i == 1)
                            str = _id.Split('-')[i];
                        else str +="-"+_id.Split('-')[i];
                    }
                switch (str)
                {
                    case "ref":
                      ViewBag.navLeb = "Refinancement";
                        break;
                    case "dfx":
                      ViewBag.navLeb = "DFX";
                        break;
                    case "aut":
                      ViewBag.navLeb = "Autres devises";
                        break;
                    case "fp":
                      ViewBag.navLeb = "Fonds propres";
                        break;
                    case "usd":
                      ViewBag.navLeb = "USD";
                        break;
            ...
            var data = GetDossiersFolder(clientId,fournisseurId:idFournisseur,siteId:siteId,motcle:_id);
            ...
            try
            {
                if(!string.IsNullOrEmpty(cat) || ViewBag.navigation!="archives" && idDossier==0)
                {
                    var tab = _id.Split('-');
                    if (tab.Length>=2)
                    {
                        if (tab[0]=="client")
                        {
                            data = data.Where(d => d.ClientName == tab[1]).ToList();
                        }
                        else if (tab[0]=="annee_mois")
                        {
                            int annee = Convert.ToInt32(tab[1]);
                            int mois = Convert.ToInt32(tab[2]);
                            data = data.Where(d => d.DateArchivage.Year == annee && d.DateArchivage.Month == mois).ToList();
                        }
                        else if (tab[0]=="annee")
                        {
                            int annee = Convert.ToInt32(tab[1]);
                            data = data.Where(d => d.AnneeArchivage == annee).ToList();
                        }
                        else if (tab[0]=="agence")
                        {
                            data = data.Where(d => d.GetAgenceName == tab[1]).ToList();
                        }
                        else if (tab[0]=="dev")
                        {
                            data = data.Where(d => d.Devise == tab[1]).ToList();
                        }
                    }
                    ...
                    if (dxk)
                        ViewBag.serachString = !string.IsNullOrEmpty(serachString) ? $"Supprimer par : {serachString}" : "";
            ...
            else
            {
                ...
                try
                {
                    foreach (var item in db.GetCompteBanqueCommerciales.Where(d => d.IdStructure == idSite || est_admin))
                ...
                foreach (var item in db.GetBanqueClients.Where(c => c.IdSite == idSite || est_admin))
                    try
                    {
                ...
                if (dxk)
                {
                    ...
                    {
                        ViewBag.erreur = "Impossible de supprimer toutes les données, veuillez selectionner une catégorie de dossiers à supprimer";
                        return PartialView("~/Views/Archivage/Donnees_banque.cshtml", new List<FileItemModel>());
                    }
                    else
                    {
                        foreach (var item in models)
                        {
                            try
                            {
                                db.GetDossiers.Find(item.Id).Remove(db);
                            }
                            catch (Exception)
                            { }
                        }
        ...
        public ActionResult GetFoldersData(int clientId, int fournisseurId = 0, double montant = 0, int deviseId = 0, int siteId = 0, string gestionnaireId = ""
            , DateTime0 dateDepot = null, DateTime0 dateEnvoiBeac = null, DateTime0 dateObtensionDevise = null, DateTime0 dateArchivage = null,string motcle="")
        {
            IniteData();
      
            return PartialView(GetDossiersFolder(clientId,fournisseurId,montant,deviseId,siteId,gestionnaireId,dateDepot,dateEnvoiBeac,dateObtensionDevise,dateArchivage,motcle:motcle));
        }
            ...
            , DateTime0 dateDepot=null,DateTime0 dateEnvoiBeac=null, DateTime0 dateObtensionDevise=null, DateTime0 dateArchivage=null,string motcle="")
        {
            ...
            bool banque_supp = true, est_admin = false;

            if ((string)Session["userType"] == "CompteClient")
            { client_supp = false; banque_supp = false; }
            else
            { banque_supp = false; }
            if ((bool)Session["EstAdmin"]) est_admin = true;

            if (clientId!=0)
                dossiers = db.GetDossiers.Where(d => d.EtapesDosier == 26 && d.ClientId == clientId && (d.SupprimeClient==client_supp || d.SupprimeBanque==banque_supp)).ToList();
            else
                dossiers = db.GetDossiers.Where(d => d.EtapesDosier == 26 &&( (d.IdSite== siteId && (d.SupprimeClient == client_supp || d.SupprimeBanque == banque_supp))||est_admin)).ToList();
            ...
            if (siteId != 0)
                dossiers = dossiers.Where(d => d.IdSite == siteId || est_admin).ToList();
                    ...
                    try
                    {
                        model.Add(
                           new FileItemModel()
                           {
                               ...
                               Name = item.ToString(cl_bq,motcle),
                               ...
                               IdDevise=(int)item.DeviseMonetaireId,
                               DFX6FP6BEAC = item.DFX6FP6BEAC,
                               Date_Etape22 = item.Date_Etape22 != null ? item.Date_Etape22 : item.Date_Etape23,
                               MoisTraite = item.Date_Etape22 != null ? item.Date_Etape22.Value.Month : item.Date_Etape21.Value.Month,
                               DfxId = item.DfxId,
