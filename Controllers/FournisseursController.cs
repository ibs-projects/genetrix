    ...
    public class FournisseursController : Controller
    {
        ...
        public async Task<ActionResult> Index(int? cl)
        {
            ...
            try
            {
                ...
                {
                    ViewBag.CreerBenef = user.CreerBenef;
                    ViewBag.ModifBenef = user.ModifBenef;
                    ViewBag.SuppBenef = user.SuppBenef; 
                }
            }
            catch (Exception)
            { }
        ...
        public async Task<ActionResult> Details(int? id)
        {
            ...
            Fournisseurs fournisseurs = await db.GetFournisseurs
                                    .Include(d=>d.RCCM)
                                    .Include(d=>d.ListeGerants)
                                    .Include(d=>d.CopieStatuts)
                                    .Include(d=>d.ListeAyantDroits)
                                    .Include(d=>d.ProcesVerbalNommantDirigeants)
                                    .Include(d=>d.CarteIdentiteDirigeants)
                                    .Include(d=>d.FicheKYCBenefi)
                                    .Include(d=>d.PriseActeDeclarationCompte)
                                    .Include(d=>d.JustifDomicileBenefi)
        ...
        public ActionResult Create()
        {
            ...
            try
            {
                if (!user.CreerBenef && client.ModeRestraint)
                    return RedirectToAction("Index", "Index", new { msg = "Vous ne pouvez pas créer un fournisseur, Veuillez contacter votre administrateur" });
            }
            catch (Exception)
            { }
            client = null;
        ...
        public async Task<ActionResult> GetCompte(int fourId,string numCmpt="")
        {
            ...
            {
                var ff = await db.GetFournisseurs.FindAsync(fourId);
                var dd = ff.CompteBeneficiaires;
                if (dd == null || dd.Count == 0)
                {
                    dd = new List<NumCompteBeneficiaire>();
                    dd.Add(new NumCompteBeneficiaire()
                    {
                        Fournisseur=ff
                    });
                }
                return Json(from d in dd select new {
                    FournisseurNom = d.Fournisseur.Nom,
                    FournisseurPays = d.Fournisseur.Pays,
                    FournisseurAdresse = d.Fournisseur.Adresse,
                    d.Id, d.Cle, d.AdresseBenf, d.NomBanque, d.Numero, d.CodeAgence, d.CodeEts, d.Nom, d.Adresse , d.CodeSwift }, JsonRequestBehavior.AllowGet);
            }
            else
            {
                ...
                {
                    var ff = await db.GetFournisseurs.FindAsync(fourId);
                    d = new NumCompteBeneficiaire()
                    {
                        Fournisseur = ff
                    };
                }
                return Json(new { FournisseurNom=d.Fournisseur.Nom,
                    FournisseurPays = d.Fournisseur.Pays,
                    FournisseurAdresse = d.Fournisseur.Adresse, 
                    d.Cle,d.AdresseBenf, d.NomBanque, d.Numero, d.CodeAgence,
                    d.CodeEts, d.Nom, d.Adresse, d.CodeSwift }, JsonRequestBehavior.AllowGet);
            }
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> CreateWithDossier([Bind(Include = "IdDossier,Nom,CodeEts,Tel2,Tel1,ClientId,Pays,Adresse,Ville,Email")] Fournisseurs fournisseurs)
        {
            if (ModelState.IsValid)
            {
                fournisseurs.ClientId = Convert.ToInt32(Session["clientId"]);
                db.GetFournisseurs.Add(fournisseurs);
                await db.SaveChangesAsync();
                if (fournisseurs.IdDossier > 0)
                    return RedirectToAction("Edit", "Dossiers", new { id = fournisseurs.IdDossier, etapeData = 0 });
                return RedirectToAction("Index");
            }
        ...
        public async Task<ActionResult> Edit(int? id)
        {
            ...
            try
            {
                if (!user.ModifBenef && client.ModeRestraint)
                    return RedirectToAction("Index", "Index", new { msg = "Vous ne pouvez pas éditer un fournisseur, Veuillez contacter votre administrateur" });
            }
            catch (Exception)
            { }
        ...
        public async Task<ActionResult> Delete(int? id)
        {
            ...
            try
            {
                if (!user.SuppBenef && client.ModeRestraint)
                    return RedirectToAction("Index", "Index", new { msg = "Vous ne pouvez pas supprimer un fournisseur, Veuillez contacter votre administrateur" });
            }
            catch (Exception)
            { }
            client = null;
