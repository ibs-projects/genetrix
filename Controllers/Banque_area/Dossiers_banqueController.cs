...
namespace genetrix.Controllers
{
    ...
    public class Dossiers_banqueController : Controller
    {
        ...
        // GET: Dossiers1
        public async Task<ActionResult> Index(string st="",string comp="",string par="",int? referenceId=null,string interf="",string adddfx="n")
        {
            if (Session==null)
                return RedirectToAction("login", "auth");
            ViewBag.adddfx = adddfx;
            //var b1 = 0;
            //var tt = 1 / b1;
                    ...
                    if ((string)Session["userType"] == "CompteBanqueCommerciale")
                    {
                        ...
                        else
                        {
                            ...
                            if (st == "refinancement") dfx_ref = 2;
                             dd = VariablGlobales.UserBanqueDossiers(db,db.GetDossiers.ToList(),site,role,gesId, estBackoffice: estbackof, montantDfx: banque.MontantDFX, dfx_ref:dfx_ref, banqueID: banque.Id);
                        }
                    }
                    ...
                    if (string.IsNullOrEmpty(comp))
                    {
                        ...
                        switch (st)
                        {
                            ...
                            case "traités":
                                ...
                                break;
                            case "traite-dfx":
                                getDossiers.AddRange(dd.Where(d => (d.EtapesDosier == 22 || d.EtapesDosier == 23) && d.DFX6FP6BEAC==1).ToList());
                                st = "traités (DFX)";
                                ViewBag.par = "traite=Tout;traite-dfx=Traités (DFX);traite-ref=Traités (Refinancement);traite-fp=Fond proptres";
                                break;
                            case "traite-ref":
                                getDossiers.AddRange(dd.Where(d => (d.EtapesDosier == 22 || d.EtapesDosier == 23) && d.DFX6FP6BEAC==3).ToList());
                                ...
                                break;
                            case "traite-fp":
                                getDossiers.AddRange(dd.Where(d => (d.EtapesDosier == 22 || d.EtapesDosier == 23) && d.DFX6FP6BEAC==2).ToList());
        ...
        public ActionResult ApurementListeView(int iddossier)
        {
            return PartialView("~/Views/Index/ApurementListeView.cshtml");
        }
        ...
        public ActionResult GetcvsFile(FormCollection form)
        {
            ...
            return File(exportFilePath, "text/xlsx");
        }
        ...
        public ActionResult GetcvsFile(int dfx_iden)
        {
            ...
            using (ExcelPackage excelPackage = new ExcelPackage(fi))
            {
               ...
               firstWorksheet.Cells["A10"].Value= "Référence MT 298 : "+ reference;
                ...
                try
                {
                    firstWorksheet.Cells["G11"].Value = dfx.Montant;
                    firstWorksheet.Cells["I11"].Value = dfx.MontantCV;
                }
                catch (Exception)
                {}
                //Save your file
            ...
            try
            {
                dfx.Telechargements +=1;
                dfx.NbrTelechargement +=1;
                db.SaveChanges();
            }
            catch (Exception)
            { }
            return File(exportFilePath, "text/xlsx", "DFX420_du_"+dateDebut + " AU " + dateFin + ".xlsx") ;
        }
        ...
        public ActionResult GetcvsFileByEmail(int dfx_iden,string emails)
        {
            ...
            using (var memoryStream = new MemoryStream())
            {
                using (var ziparchive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
                {
                    ...
                    {
                        //CSV
                        ziparchive.CreateEntryFromFile(exportFilePath, "DFX420_du_" + dateDebut.Replace("/","") + "_AU_" + dateFin.Replace("/", "") + ".xlsx");;
                    }
                    catch (Exception e)
                    { }
                }
        ...
        public async Task<ActionResult> Details(int? id,string unit="")
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    //var rccm = db.GetDocumentations.FirstOrDefault(d => d.Nom == "RCCM" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.RCCM = db.GetDocumentations.FirstOrDefault(d => d.Nom == "RCCM" && d.IdFournisseur == id);
                    //dossier.Fournisseur.ListeGerants = db.GetDocumentations.FirstOrDefault(d => d.Nom == "ListeGerants" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.CopieStatuts = db.GetDocumentations.FirstOrDefault(d => d.Nom == "CopieStatuts" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.ListeAyantDroits = db.GetDocumentations.FirstOrDefault(d => d.Nom == "ListeAyantDroits" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.ProcesVerbalNommantDirigeants = db.GetDocumentations.FirstOrDefault(d => d.Nom == "ProcesVerbalNommantDirigeants" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.CarteIdentiteDirigeants = db.GetDocumentations.FirstOrDefault(d => d.Nom == "CarteIdentiteDirigeants" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.FicheKYCBenefi = db.GetDocumentations.FirstOrDefault(d => d.Nom == "FicheKYCBenefi" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.PriseActeDeclarationCompte = db.GetDocumentations.FirstOrDefault(d => d.Nom == "PriseActeDeclarationCompte" && d.IdFournisseur == dossier.FournisseurId);
                    //dossier.Fournisseur.JustifDomicileBenefi = db.GetDocumentations.FirstOrDefault(d => d.Nom == "JustifDomicileBenefi" && d.IdFournisseur == dossier.FournisseurId);
               
                    var docsFour = db.GetDocumentations.Where(d =>d.IdFournisseur == dossier.FournisseurId);
                    var rccm = docsFour.FirstOrDefault(d => d.Nom == "RCCM");
                    dossier.Fournisseur.RCCM = docsFour.FirstOrDefault(d => d.Nom == "RCCM");
                    dossier.Fournisseur.ListeGerants = docsFour.FirstOrDefault(d => d.Nom == "ListeGerants");
                    dossier.Fournisseur.CopieStatuts = docsFour.FirstOrDefault(d => d.Nom == "CopieStatuts");
                    dossier.Fournisseur.ListeAyantDroits = docsFour.FirstOrDefault(d => d.Nom == "ListeAyantDroits");
                    dossier.Fournisseur.ProcesVerbalNommantDirigeants = docsFour.FirstOrDefault(d => d.Nom == "ProcesVerbalNommantDirigeants");
                    dossier.Fournisseur.CarteIdentiteDirigeants = docsFour.FirstOrDefault(d => d.Nom == "CarteIdentiteDirigeants");
                    dossier.Fournisseur.FicheKYCBenefi = docsFour.FirstOrDefault(d => d.Nom == "FicheKYCBenefi");
                    dossier.Fournisseur.PriseActeDeclarationCompte = docsFour.FirstOrDefault(d => d.Nom == "PriseActeDeclarationCompte");
                    dossier.Fournisseur.JustifDomicileBenefi = docsFour.FirstOrDefault(d => d.Nom == "JustifDomicileBenefi");
                    docsFour = null;
                }
                catch (Exception)
                {}
                try
                {
                    var docsClient = db.GetDocumentations.Where(d => d.ClientId == dossier.ClientId);
                    dossier.Client.PlanLSS = docsClient.FirstOrDefault(d =>d.Nom == "PlanLSS");
                    dossier.Client.FicheKYC = docsClient.FirstOrDefault(d => d.Nom == "FicheKYC");
                    dossier.Client.RCCM_Cl = docsClient.FirstOrDefault(d =>  d.Nom == "RCCM");
                    dossier.Client.Statut = docsClient.FirstOrDefault(d =>  d.Nom == "Statut");
                    dossier.Client.ProcesVerbal = docsClient.FirstOrDefault(d =>  d.Nom == "ProcesVerbal");
                    dossier.Client.EtatFinanciers = docsClient.FirstOrDefault(d =>  d.Nom == "EtatFinanciers");
                    dossier.Client.AtestationHinneur = docsClient.FirstOrDefault(d =>  d.Nom == "AtestationHinneur");
        ...
        public async Task<ActionResult> Archiver(int id)
        {
            try
            {
                var doss = db.GetDossiers.Find(id);
                ...
                await MailFunctions.ChangeEtapeDossier((int)idbanque, doss.IdGestionnaire, 26, doss, db, structure);
                db.SaveChanges();
            }
            catch (Exception)
            { }
        ...
        public async Task<ActionResult> ModifEtatDossierNJS(DossierChange model)
        {
            ...
            else if(model.etat==232)
            {
                return RedirectToAction("indexBanque", "Index", new { panel = "apurement" });
            }
        ...
        public JsonResult ChangeWf(int doss)
        {
            ...
            if (dossier.DFX6FP6BEAC!=2)
            {
                ...
                if (!dossier.Client.AttestNonDefautValide && dossier.Client.AttestationSurHonneur == null)
                    return Json($"Impossible de traiter le dossier sur fonds propres, l'attestations de non defaut d'apurement et l'attestation sur l'honneur du client sont invalides!", JsonRequestBehavior.AllowGet);
                else if (!dossier.Client.AttestNonDefautValide)
                    return Json($"Impossible de traiter le dossier sur fonds propres, attestation de non defaut d'apurement invalide.", JsonRequestBehavior.AllowGet);
                else if (dossier.Client.AttestationSurHonneur == null)
                {
                    return Json($"Impossible de traiter le dossier sur fonds propres, aucune attestation sur l'honneur valide n'est trouvée.", JsonRequestBehavior.AllowGet);
                }
            }
        ...
        public async Task<JsonResult> GetRefCorrespondantes(int DossierId,int fournisseurId,int deviseId,int clientId)
        {
            ...
            //refinancement
            if (doss.DFX6FP6BEAC==3)
            {
                db.GetDossiers.Include(d => d.ReferenceExterne).Where(d =>d.DFX6FP6BEAC==3 && d.EtapesDosier < 15 && d.EtapesDosier >= 13 && d.ClientId == clientId && d.FournisseurId == fournisseurId && d.DeviseMonetaireId == deviseId).ToList().ForEach(d =>
                {
            ...
            else if (doss.DFX6FP6BEAC==1)
            {
                db.GetDossiers.Include(d => d.DFX).Where(d =>d.DFX6FP6BEAC==1 && d.EtapesDosier < 15 && d.EtapesDosier >= 13 && d.ClientId == clientId && d.FournisseurId == fournisseurId && d.DeviseMonetaireId == deviseId).ToList().ForEach(d =>
                {
                    if (d.DfxId == null)
                    {
                        dossiers.Add(d);
