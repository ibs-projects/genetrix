    ...
    public class DossiersController : Controller
    {
             ...
             HttpPostedFileBase ImageQuittancePay = null, HttpPostedFileBase ImageDocumentTransport = null)// IEnumerable<HttpPostedFileBase> images)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var bank = db.Structures.Find(banqueId);
                if (dossier.Montant <= bank.MontantDFX)
                    dossier.Categorie = 0;
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (dossier.DateDepotBank == new DateTime())
                        dossier.DateDepotBank = DateTime.Now;
                    //var IdAgentResponsableDossier = db.GetDossiers.Find(dossier.Dossier_Id).IdAgentResponsableDossier;
                    //if(IdAgentResponsableDossier!=null)
                    //    dossier.IdAgentResponsableDossier = IdAgentResponsableDossier;
        ...
        public ActionResult Transmis(int id)
        {
            if (id > 0)
            {
                var doss = db.GetDossiers.Include(d=>d.Site).FirstOrDefault(d=>d.Dossier_Id== id);
                var site = db.Structures.FirstOrDefault(d=>d.Id== doss.IdSite);
                var idbanque = site.BanqueId(db);
                doss.DateModif = DateTime.Now;
                doss.DateDepotBank = DateTime.Now;
                MailFunctions.ChangeEtapeDossier((int)idbanque, 1, doss, db, site);
            }
