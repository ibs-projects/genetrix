...
namespace genetrix.Controllers
{
    public class ReferenceBanquesController : Controller
    {
        ...
        public ActionResult Index(bool reference=false,string st="",string comp="")
        {
            if (Session == null)
            ...
            var dossiers = VariablGlobales.UserBanqueDossiers(db, site, role, banqueID, gesId,montantDfx: baqnue.MontantDFX,dfx_ref:2);
                ...
                switch (type)
                {
                    ...
                    case "bd":
                        ...
                        var comptes = from c in corresp.GetCompteNostros
                                      select new { c.Id,c.Libellé,c.Numero };
        ...
        public async Task<ActionResult> Details(int? id)
        {
            ...
            var structure = db.Structures.Find(user.IdStructure);
            ...
            if (IdRef > 0)
            {
                ...
                if (mt==0 && fileCourrier != null)
                {
                    if (!string.IsNullOrEmpty(fileCourrier.FileName))
                    {
                        ...
                        imageModel.Url = Path.Combine(CreateNewFolderDossier(reference.ClientId.ToString(), reference.FirstDossier.Intitulé), chemin);
                ...
                //MT
                else if (mt==1 && fileCourrier != null)
                {
                    if (!string.IsNullOrEmpty(fileCourrier.FileName))
                    {
                        var imageModel = new genetrix.Models.ImageDocumentAttache();
