...
namespace eApurement.Controllers
{
    ...
    public class Index2Controller : Controller
    {
        ...
        public Index2Controller(ApplicationUserManager userManager)
        {
        ...
        public ActionResult IndexBanque(string panel="")
        {
            ...
            var _model = model.infoDocBanque(6);
            ...
            List<int> etapes = new List<int>();
            Session["str3"] = panel;
            if (panel.Contains(","))
            {
                var elts = panel.Split(',');
                for (int i = 1; i < elts.Length; i++)
                {
                    try
                    {
                        etapes.Add(int.Parse(elts[i]));
                    }
                    catch (Exception)
                    {}
                }
                var intervalle = elts[0];
                Session["str1"] = intervalle.Split('_')[0];
                Session["str2"] = intervalle.Split('_')[1];
            }
            else
            {
                Session["str1"] = panel.Split('_')[0];
                Session["str2"] = panel.Split('_')[1];
            }
            int etapemin = int.Parse(Session["str1"].ToString()),
            etapemax = int.Parse(Session["str2"].ToString());
            ...
            if (!(Session["user"] as CompteBanqueCommerciale).EstAdmin)
            {
                foreach (var item in site.Composants)
                {
                    try
                    {
                        if (role.GetIHMs != null || role.GetIHMs.Count() > 0 || role.GetIHMs.FirstOrDefault(i => i.ComposantId == item.ComposantId) != null)
                            continue;
                    }
                    catch (Exception)
                    { }
                    //composants.Add(item.Composant);
                    if (item.Composant.Numero >= etapemin && etapemax >= item.Composant.Numero || etapes.Contains(item.Composant.Numero))
                        if (item.Composant.Type == eApurement.Models.Type.lien_bouton)
                        {
                            ...
                            try
                            {
                                if (item.Composant.NumeroMax > item.Composant.NumeroMin)
                                {
                                    int _nbr = 0; double _percent = 0; DateTime date = default;
                                    for (int i = item.Composant.NumeroMin; i < item.Composant.NumeroMax; i++)
                                    {
                                        if (_model.Keys != null && _model.Keys.Contains(i))
                                        {
                                            _nbr += _model[i].Nbr;
                                ...
                                else
                                {
                                    ...
                                    if (_model.Keys != null && _model.Keys.Contains(item.Composant.Numero))
                                    {
                                        infoDocAcueil= new InfoDocAcueil()
                                        {
                                            Date = _model[item.Composant.Numero].Date,
                                            Nbr = _model[item.Composant.Numero].Nbr,
                                            Percentage = _model[item.Composant.Numero].GetPercentage(),
                                            EtapeDossier= item.Composant.Numero
                                        };
                                    }
            ...
            else
            {
                ...
                foreach (var item in db.GetComposants)
                {
                    infoDocAcueil = new InfoDocAcueil();
                    try
                    {
