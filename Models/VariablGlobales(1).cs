...
namespace eApurement.Models
{
    public class VariablGlobales
    {
        ...
        public static List<Dossier> UserBanqueDossiers(ApplicationDbContext db,Structure site,XtraRole role,int banqueID,string agentId)
        {
            ...
            /*Structure permisions*/ && d.EtapesDosier >= site.NiveauDossier  
                                    ).ToList().ForEach(d =>
                                    {
                                        ///Provisoir
                                        ///
        ...
        /// <returns></returns>
        public static bool AfficheAction(int? etapeDossier,int min,int max,int etapeMaxi=0)
        {
            bool ecrire = false;
            try
            {
                ...
                {
                    if (etapeDossier >= min && etapeDossier <= max)
                    {
                        ecrire = true;
                    }
                }
