...
function NumberToLetter(nombre, devise) {
	var str = nombre + "";
	if (str.includes('.')) {
		nombre = Number(str.split('.')[0]);
		var intPart = Number(str.split('.')[1]) / 100;
		alert(intPart)
    }
	...
	if (intPart > 0) {
		return numberToLetter + ' '+devise+" et " + NombreCentime2(intPart);
	}
	else
		return numberToLetter.re + ' ' + devise;
}


function NombreCentime2(nombreApresLaVirgule) {
	nombreApresLaVirgule = Number(nombreApresLaVirgule*100);
	return nombreApresLaVirgule == 1 ? nombreApresLaVirgule + " centime" : nombreApresLaVirgule+ " centimes";
}